...
const CONFIRM_LEAVE_CUSTOM_ID = "btn_confirm_leave"; // เพิ่ม Custom ID สำหรับปุ่มยืนยันลุกจากโต๊ะ
...
async function handleJoinQueue(interaction) {
  ...
  if (partnerIndex !== -1) {
    ...
    try {
      ...
      // แท็กห้องแชทในข้อความจับคู่สำเร็จ
...
async function handleLeaveTable(interaction) {
  ...
  // 1. เช็คว่าโต๊ะนี้ถูกล็อคจากการ Report หรือเปล่า
  ...
  // 2. เช็ค Cooldown 1 นาที (60000 ms)
  ...
  // 3. ส่งปุ่มยืนยัน (ไม่ใช้ ephemeral)
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      // ฝัง User ID ลงไปในปุ่มเพื่อเช็คว่าใครเป็นคนกด
...
async function handleConfirmLeave(interaction) {
  // แยกเอา ID ของคนที่ควรกดได้ออกมาเช็ค
  const [, targetUserId] = interaction.customId.split(":");

  // ถ้าคนที่กดปุ่ม ไม่ใช่คนที่กดลุกจากโต๊ะตั้งแต่แรก
  ...
  try {
    ...
    if (actionMsg) {
      const disabledReportRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId(LEAVE_TABLE_CUSTOM_ID)
          .setLabel("🚪 ลุกจากโต๊ะ (ถูกระงับ)") // อัปเดตข้อความเพื่อแจ้งเตือน
          .setStyle(ButtonStyle.Danger)
          .setDisabled(true), // ปิดการใช้งานปุ่มลุกจากโต๊ะ
        new ButtonBuilder()
          .setCustomId(REPORT_USER_CUSTOM_ID)
...
function setupSecretChat(client) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    } else if (interaction.customId.startsWith(CONFIRM_LEAVE_CUSTOM_ID + ":")) { // รองรับปุ่มยืนยัน
